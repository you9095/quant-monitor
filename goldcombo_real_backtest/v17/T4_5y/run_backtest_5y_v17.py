...
# -*- coding: utf-8 -*-
"""
T4 · V17_LowFreqBreakout 极低频突破版策略类跑 1950 只沪深 A 股 5Y 真回测 (2026-08-16)
- 引擎: backtrader 1.9.78.123 (非闭式估算代理)
- 池: 沪深 A 股 1950 只 (排除 688/300/8/4, ashare_pool.json 预生成)
- 时间窗: 2021-08-14 ~ 2026-08-14 (5Y)
- 策略类: GoldComboV17_LowFreqBreakout (用户原版极低频突破, 一字不差 import, 不允许任何修改)
- 入场: 收盘价 > 120日 最高价(-1) + MA20>MA60>MA120 (半年突破 + 多头序双条件)
- 出场: 15% 硬止损 OR 20% 峰值回撤止盈 OR 收盘价 < MA20 (短期生命线跌破)
- 仓位: 95% 集中持仓 (per_pos_pct=0.95, 单只满仓)
- 资金: setcash(50000.0) 锁死 (用户原话硬约束)
- 输出: T4_5y/baseline_ashare_real_5y_v17.json

用户原话硬约束 (2026-08-16):
1. "使用 GoldComboV17_LowFreqBreakout, 5万本金锁死, 1950沪深池, 5Y窗口, 类名不得改, 一字不差" → 一字不差 import, 不允许任何修改
2. "严禁添加任何外部 hold/lock/lockday 逻辑" → V17 内部纯 4 指标 (Highest 120 + SMA 20/60/120), 无外部 lock
3. "硬性规则全内嵌" → 所有规则 (半年突破 + MA多头序入场 / 15%硬止损 + 20%峰值回撤 + 破MA20离场 / 95%集中持仓) 都在类内, 不外置
4. "setcash(50000.0) 锁死" → 每只子账户强制 5万本金
5. "5Y 沪深池回测" → 5Y 数据期 2021-08-14 ~ 2026-08-14, 1950 只沪深池
6. "底线目标: 年化>30% & 5年<1000笔" → V17 极低频设计目标

V17 用户原版来源: ~/Downloads/股票筛选项目/自己写量化策略和脚本/混元三黄金组合V17_LowFreqBreakout.rtf
V17 已一字不差写入: /Users/junze/quant-monitor-local/strategies/goldcombo/goldcombo_strategy_ashare_v17.py
git commit SHA: e2d105b
"""
import json
import os
import sys
...
OUT_DIR = '/Users/junze/goldcombo_real_backtest/v17/T4_5y'
OUT_JSON = os.path.join(OUT_DIR, 'baseline_ashare_real_5y_v17.json')
...
# 单股独立子账户: 用户原话锁死 5万/股 (复刻 V12/V13/V14/V16 配置)
...
from strategies.goldcombo.goldcombo_strategy_ashare_v17 import GoldComboV17_LowFreqBreakout  # noqa: E402
...
def run_single_stock(code: str, start: str, end: str, capital: float) -> Optional[Dict]:
    ...
    # ===== V17 用户原版, 不传任何额外参数 (硬约束: 不准擅自修改 7 参数) =====
...
def main():
    ...
    log(f'[T4] V17 用户原版 · GoldComboV17_LowFreqBreakout (范式革命 2.0: 长周期 120/60/20 + 集中持仓 95%, 一字不差 import)')
    ...
    log(f'[T4] V17 7 参数: break_n=120/ma_short=20/ma_mid=60/ma_long=120/trail_sl=0.20/hard_sl=0.15/per_pos_pct=0.95')
    log(f'[T4] V17 4 指标: Highest(120日高)/SMA(20)/SMA(60)/SMA(120) — 极简长周期')
    log(f'[T4] 入场 (半年突破 + 多头序): 收盘价 > 120日 最高价(-1) + MA20>MA60>MA120')
    log(f'[T4] 出场: 15% 硬止损 OR 20% 峰值回撤止盈 OR 收盘价 < MA20 (短期生命线跌破)')
    log(f'[T4] 仓位 (95% 集中持仓): cash × 0.95 → 单只满仓, 集中火力')
    log(f'[T4] 过滤: price < 3.0 OR MA120=NaN OR Highest=NaN → return (三过滤)')
    log(f'[T4] 用户底线目标 (V17 特有): 年化>30% & 5年<1000笔')
    ...
    # V17 策略类 sha256 (用户原版对照)
    import hashlib
    v17_path = '/Users/junze/quant-monitor-local/strategies/goldcombo/goldcombo_strategy_ashare_v17.py'
    with open(v17_path, 'rb') as f:
        v17_sha256 = hashlib.sha256(f.read()).hexdigest()

    summary = {
        'strategy_id': 'goldcombo',
        'strategy_version': 'V17_LowFreqBreakout (GoldComboV17_LowFreqBreakout 用户原版极低频突破, 2026-08-16)',
        'strategy_name': '黄金组合A · 沪深 A 股 (排除科创+创业) · V17 极低频突破版',
        'data_period': '5Y',
        ...
        'strategy_file_sha256': v17_sha256,
        'entry_logic_v17': '极低频突破: 收盘价 > 120日 最高价(-1) + MA20>MA60>MA120(多头序双条件)',
        'exit_logic_v17': '15% 硬止损 + 20% 峰值回撤止盈 + 收盘价 < MA20(短期生命线跌破)',
        'position_sizing_v17': '95% 集中持仓: cash × 0.95 → 单只满仓 (per_pos_pct=0.95)',
        'config': {
            'initial_capital': INITIAL_CAPITAL,
            'effective_capital': effective_capital,
            'capital_per_stock': CAPITAL_PER_STOCK,
            'break_n': 120, 'ma_short': 20, 'ma_mid': 60, 'ma_long': 120,
            'trail_sl': 0.20, 'hard_sl': 0.15, 'per_pos_pct': 0.95,
        ...
        'comparison': {
            ...
            'v17_5y_return_pct': round(total_return_pct, 4),
            'v17_5y_trade_count': int(total_trades),
            'v17_5y_pool': n_pool,
            'v17_5y_worst_dd': round(worst_max_dd, 4),
        },
        'user_predicted': '用户原话 (2026-08-16 派单): "新的策略 开始回测"。重点观察 (沿用 V16 惯例): 总收益 / 笔数 / worst DD。用户底线目标 (V17 特有): 年化>30% & 5年<1000笔',
        'honest_declaration': (
            'V17_LowFreqBreakout 是用户上传的极低频突破版 (类名 GoldComboV17_LowFreqBreakout, 半年突破 + 多头序 + 集中持仓 95%)。'
            'V17 一字不差, 未加任何外部 hold/lock。'
            'V17 替换 V16_ChannelBreakout 旧类, 强制 setcash(50000.0) 锁死。'
            'V17 数据可比因都跑 1950 只沪深 A 股 5Y (排除 688/300 科创+创业)。'
            'V17 是 v1-v16 之外的新范式 (长周期 + 集中持仓), 与 v1-v16 的中短周期 + 分仓完全不同。'
        ),
    }
    ...
    log(f'[T4] ⭐ 笔数 (用户重点观察 2): {total_trades} (用户底线目标 <1000 笔)')
    log(f'[T4] ⭐ 最大回撤 worst (用户重点观察 3): {worst_max_dd:.4f}%')
    log(f'[T4] annualized: {annualized:.4f}% (用户底线目标 >30%)')
