...
# -*- coding: utf-8 -*-
"""
T4 · V12_LeftBuyRightSell 左买右卖混合版策略类跑 1950 只沪深 A 股 5Y 真回测 (2026-08-16)
- 引擎: backtrader 1.9.78.123 (非闭式估算代理)
- 池: 沪深 A 股 1950 只 (排除 688/300/8/4, ashare_pool.json 预生成)
- 时间窗: 2021-08-14 ~ 2026-08-14 (5Y)
- 策略类: GoldComboV12_LeftBuyRightSell (用户原版左买右卖混合, 一字不差 import, 不允许任何修改)
- 入场: C3 MACD低位金叉 + [C4/C7/C8] 至少 1 投票 (vote_min=1, 同 V10/V11)
- 出场: 30% 硬止损 (防退市,同 V10 路径 B) + 25% 峰值回撤止盈 (同 V10) + 能量终结离场 (破MA20+DMI空方反扑)
- 仓位: 单票 10% 总资 (per_pos_pct=0.10, 复刻 V10 路径 B 5000/只最优配置)
- 资金: setcash(50000.0) 锁死 (用户原话硬约束)
- 输出: T4_5y/baseline_ashare_real_5y_v12.json

用户原话硬约束 (2026-08-16):
1. "用上方 GoldComboV12_LeftBuyRightSell 替换旧类" → 一字不差 import, 不允许任何修改
2. "不准加任何外部 hold/lock" → 不允许任何外部包装/拦截/hold/lock/sl
3. "setcash(50000.0) 锁死" → 每只子账户强制 5万本金
4. "5Y 沪深池回测" → 5Y 数据期 2021-08-14 ~ 2026-08-14, 1950 只沪深池
5. "复刻 V10 路径 B 5000/只最优配置" → 10% 仓位 = 5万 × 10% = 5000/股 (per_pos_pct=0.10)

V12 用户原版来源: ~/Downloads/股票筛选项目/自己写量化策略和脚本/混元黄金优化第四版V12左买右卖混合版.py
V12 已一字不差写入: /Users/junze/quant-monitor-local/strategies/goldcombo/goldcombo_strategy_ashare_v12.py
git commit SHA: 925efd4
"""
import json
import os
import sys
...
OUT_DIR = '/Users/junze/goldcombo_real_backtest/v12/T4_5y'
OUT_JSON = os.path.join(OUT_DIR, 'baseline_ashare_real_5y_v12.json')
...
# 单股独立子账户: 用户原话锁死 5万/股 (复刻 V11 配置, V12 策略类内 per_pos_pct=0.10 → 单票 5000/只)
...
from strategies.goldcombo.goldcombo_strategy_ashare_v12 import GoldComboV12_LeftBuyRightSell  # noqa: E402
...
def run_single_stock(code: str, start: str, end: str, capital: float) -> Optional[Dict]:
    ...
    # ===== V12 用户原版, 不传任何额外参数 (硬约束: 不准擅自修改 8 个参数) =====
...
def main():
    ...
    log(f'[T4] V12 用户原版 · GoldComboV12_LeftBuyRightSell (左买右卖混合, 一字不差 import)')
    ...
    log(f'[T4] V12 8 参数: cci_thresh=-70.0/di_neg=20.0/di_pos=15.0/vote_min=1/price_min=3.0/per_pos_pct=0.10/hard_sl=0.30/trail_sl=0.25')
    log(f'[T4] 入场: C3 MACD 零下金叉 + [C4/C7/C8] ≥ 1 (极敏感) | 出场: 30% 防退市硬止损 + 25% 峰值回撤 + 能量终结(破MA20+DMI空方反扑+DI<10且-DI>25)')
    log(f'[T4] 复刻 V10 路径 B: per_pos_pct=0.10 = 5000/只最优配置')
    ...
    # V12 策略类 sha256 (用户原版对照)
    import hashlib
    v12_path = '/Users/junze/quant-monitor-local/strategies/goldcombo/goldcombo_strategy_ashare_v12.py'
    with open(v12_path, 'rb') as f:
        v12_sha256 = hashlib.sha256(f.read()).hexdigest()

    summary = {
        'strategy_id': 'goldcombo',
        'strategy_version': 'V12_LeftBuyRightSell (GoldComboV12_LeftBuyRightSell 用户原版, 2026-08-16)',
        'strategy_name': '黄金组合A · 沪深 A 股 (排除科创+创业) · V12 左买右卖混合版',
        'data_period': '5Y',
        ...
        'strategy_file_sha256': v12_sha256,
        'entry_logic_v12': 'C3 必选 (MACD 低位金叉) + [C4 (BOLL 开口) / C7 (CCI<-70) / C8 (DMI 空方)] ≥ 1 投票 (同 V10/V11)',
        'exit_logic_v12': '30% 硬止损 (同 V10 路径 B 宽容) + 25% 峰值回撤止盈 (同 V10) + 能量终结离场 (右移卖点: 价格破 MA20 且 DMI 空方反扑 +DI<10 且 -DI>25)',
        'config': {
        ...
        'comparison': {
            ...
            'v12_5y_return_pct': round(total_return_pct, 4),
            'v12_5y_trade_count': int(total_trades),
            'v12_5y_pool': n_pool,
            'v12_5y_worst_dd': round(worst_max_dd, 4),
        },
        'user_predicted': '用户原话 (2026-08-16 派单): 重点对比 V10 路径 B (+1.5991%). 收益应 ≥ +1.6% 且回撤 ≤ -15%。若 V12 仍不达"高收益 (年化>5%)", 下一步迭代 V13: 直接将买点改成原始卖点组 (DMI多方+MACD水上) 全满足才入场 (纯右侧), 彻底放弃左侧',
        'honest_declaration': (
            'V12_LeftBuyRightSell 是用户上传的左买右卖混合版 (类名 GoldComboV12_LeftBuyRightSell)。'
            'V12 一字不差, 未加任何外部 hold/lock (用户原话硬约束)。'
            'V12 替换 V11_EnergyPeak 旧类, 强制 setcash(50000.0) 锁死 + 10% 仓位复刻 V10 路径 B 5000/只最优配置。'
            'V12 vs V11 数据可比因都跑 1950 只沪深 A 股 5Y (排除 688/300 科创+创业)。'
            'V12 与 V10 路径 B 的核心差异: 出场新增"能量终结"右移卖点(破MA20+DMI空方反扑+DI<10且-DI>25),'
            '把"卖得太早"问题向右移延长主升浪持仓时间, 同时保留 V10 路径 B 的 5000/只单票仓位。'
        ),
    }
