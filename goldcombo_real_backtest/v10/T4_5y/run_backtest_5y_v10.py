...
# -*- coding: utf-8 -*-
"""
T4 · V10_HighYield 激进左翼高收益版策略类跑 1950 只沪深 A 股 5Y 真回测 (2026-08-16)
- 引擎: backtrader 1.9.78.123 (非闭式估算代理)
- 池: 沪深 A 股 1950 只 (排除 688/300, ashare_pool.json 预生成)
- 时间窗: 2021-08-14 ~ 2026-08-14 (5Y)
- 策略类: GoldComboV10_HighYield (用户原版激进左翼, 一字不差 import, 不允许任何修改)
- 入场: C3 MACD低位金叉 + [C4/C7/C8] 至少 1 投票 (vote_min=1, 极敏感)
- 出场: 30% 硬止损 (防退市) + 25% 峰值回撤止盈 + CCI>200 泡沫顶
- 资金分配: 等权 1/N (子账户 500 元, 共 975000; per_pos_pct=0.20)
- 输出: T4_5y/baseline_ashare_real_5y_v10.json

用户原话三硬约束 (2026-08-16):
1. "用上方 GoldComboV10_HighYield 替换旧类" → 一字不差 import, 不允许任何修改
2. "不准加任何外部 hold/lock" → 不允许任何外部包装/拦截/hold/lock/sl
3. "5Y 沪深池回测" → 5Y 数据期 2021-08-14 ~ 2026-08-14, 1950 只沪深池

V10 用户原版来源: ~/Downloads/股票筛选项目/自己写量化策略和脚本/混元黄金优化第四版V10激进左翼.py
V10 已一字不差写入: /Users/junze/quant-monitor-local/strategies/goldcombo/goldcombo_strategy_ashare_v10.py
git commit SHA: f0403796
"""
import json
import os
import sys
...
OUT_JSON = os.path.join(OUT_DIR, 'baseline_ashare_real_5y_v10.json')
OUT_LOG = os.path.join(OUT_DIR, 'raw_output.log')

INITIAL_CAPITAL = 10000.0
COMMISSION_RATE = 0.001
SLIPPAGE = 0.003  # V10 策略内 broker.set_slippage_perc(0.003) (用户原版)
...
from strategies.goldcombo.goldcombo_strategy_ashare_v10 import GoldComboV10_HighYield  # noqa: E402
...
def run_single_stock(code: str, start: str, end: str, capital: float) -> Optional[Dict]:
    ...
    # ===== V10 用户原版, 不传任何额外参数 (硬约束: 不准擅自修改 9 个参数) =====
...
def main():
    ...
    log(f'[T4] V10 用户原版 · GoldComboV10_HighYield (激进左翼高收益, 一字不差 import)')
    ...
    log(f'[T4] V10 9 参数: cci_thresh=-70.0/di_neg=20.0/di_pos=15.0/vote_min=1/price_min=3.0/per_pos_pct=0.20/hard_sl=0.30/trail_sl=0.25/cci_bubble=200.0')
    log(f'[T4] 入场: C3 MACD 零下金叉 + [C4/C7/C8] ≥ 1 (极敏感) | 出场: 30% 防退市硬止损 + 25% 峰值回撤 + CCI>200 泡沫顶')
    ...
    # V10 策略类 sha256 (用户原版对照)
    import hashlib
    v10_path = '/Users/junze/quant-monitor-local/strategies/goldcombo/goldcombo_strategy_ashare_v10.py'
    with open(v10_path, 'rb') as f:
        v10_sha256 = hashlib.sha256(f.read()).hexdigest()

    summary = {
        'strategy_id': 'goldcombo',
        'strategy_version': 'V10_HighYield (GoldComboV10_HighYield 用户原版激进左翼, 2026-08-16)',
        'strategy_name': '黄金组合A · 沪深 A 股 (排除科创+创业) · V10 激进左翼高收益版',
        'data_period': '5Y',
        ...
        'strategy_file_sha256': v10_sha256,
        'entry_logic_v10': 'C3 必选 (MACD 低位金叉) + [C4 (BOLL 开口) / C7 (CCI<-70) / C8 (DMI 空方)] ≥ 1 投票 (极敏感)',
        'exit_logic_v10': '30% 硬止损 (防退市) + 25% 峰值回撤止盈 + CCI>200 泡沫顶 (吃尽主升浪)',
        'config': {
        ...
        'comparison': {
            ...
            'v10_5y_return_pct': round(total_return_pct, 4),
            'v10_5y_trade_count': int(total_trades),
            'v10_5y_pool': len(pool_5y),
            'v10_5y_worst_dd': round(worst_max_dd, 4),
        },
        'user_predicted': '用户原话 (2026-08-16 派单): 预期收益转正 + 显著放大, 但回撤可能突破 -30%',
        'honest_declaration': (
            'V10_HighYield 是用户上传的激进左翼高收益版 (类名 GoldComboV10_HighYield)。'
            'V10 一字不差, 未加任何外部 hold/lock (用户原话硬约束)。'
            'V10 替换 V7FIXOBV 旧类 (V7FIXOBV 是右侧主升, V10 是激进左侧抄底)。'
            'V10 vs V9 数据可比因都跑 1950 只沪深 A 股 5Y (排除 688/300 科创+创业)。'
            'V7FIXOBV 沿用历史 baseline 数据, 但已不作为当前默认策略类 (f0403796 commit)。'
            'V10 sizing 公式 int(cash*0.20/(price*100))*100 限制高价股(>~20元)买不起 1 手, '
            '故茅台/五粮液等高价股 0 交易属正常策略特性, 不是 bug。'
        ),
    }
    ...
    log(f'[T4] ⭐ 最大回撤 worst (用户重点观察 2): {worst_max_dd:.4f}%')
