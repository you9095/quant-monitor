...
# -*- coding: utf-8 -*-
"""
T4 · V14_ScaleIn 左试右加版策略类跑 1950 只沪深 A 股 5Y 真回测 (2026-08-16)
- 引擎: backtrader 1.9.78.123 (非闭式估算代理)
- 池: 沪深 A 股 1950 只 (排除 688/300/8/4, ashare_pool.json 预生成)
- 时间窗: 2021-08-14 ~ 2026-08-14 (5Y)
- 策略类: GoldComboV14_ScaleIn (用户原版左试右加, 一字不差 import, 不允许任何修改)
- 入场: 首次半仓 (左试) C3 必选 + [C4/C7/C8] ≥ 1 → 买半仓 (10% 总资)
- 加仓: 持仓中 + 未加过 + 价格 > MA10 → 买另半仓 (10% 总资) → 加满 20%
- 出场: 20% 硬止损 + 25% 峰值回撤止盈 + 破 MA10 快离场 + MACD 死叉
- 仓位: 首次 5000/只 (10% 半仓) + 加满 10000/只 (20%)
- 资金: setcash(50000.0) 锁死 (用户原话硬约束)
- 输出: T4_5y/baseline_ashare_real_5y_v14.json

用户原话硬约束 (2026-08-16):
1. "使用 GoldComboV14_ScaleIn, 5万本金锁死, 1950沪深池, 5Y窗口, 类名不得改, 一字不差" → 一字不差 import, 不允许任何修改
2. "严禁添加任何外部 hold/lock/lockday 逻辑" → self.added 是 V14 内部加仓状态机, 不是外部 lock
3. "硬性规则全内嵌" → 所有规则 (买入 C3+≥1 / 加仓站上MA10 / 卖出 4 机制) 都在类内, 不外置
4. "setcash(50000.0) 锁死" → 每只子账户强制 5万本金
5. "5Y 沪深池回测" → 5Y 数据期 2021-08-14 ~ 2026-08-14, 1950 只沪深池
6. "复刻 V12 5000/只最优配置" → half_pct=0.10 = 5万 × 10% = 5000/只 (首次半仓)

V14 用户原版来源: ~/Downloads/股票筛选项目/自己写量化策略和脚本/混元三黄金组合V14_ScaleIn.rtf
V14 已一字不差写入: /Users/junze/quant-monitor-local/strategies/goldcombo/goldcombo_strategy_ashare_v14.py
git commit SHA: c27d509
"""
import json
import os
import sys
...
OUT_DIR = '/Users/junze/goldcombo_real_backtest/v14/T4_5y'
OUT_JSON = os.path.join(OUT_DIR, 'baseline_ashare_real_5y_v14.json')
...
# 单股独立子账户: 用户原话锁死 5万/股 (复刻 V12/V13 配置)
...
from strategies.goldcombo.goldcombo_strategy_ashare_v14 import GoldComboV14_ScaleIn  # noqa: E402
...
def run_single_stock(code: str, start: str, end: str, capital: float) -> Optional[Dict]:
    ...
    # ===== V14 用户原版, 不传任何额外参数 (硬约束: 不准擅自修改 7+ 参数) =====
...
def main():
    ...
    log(f'[T4] V14 用户原版 · GoldComboV14_ScaleIn (左试右加: 首次半仓 + MA10加仓 + 快卖, 一字不差 import)')
    ...
    log(f'[T4] V14 8 参数: cci_thresh=-70/di_neg=20/di_pos=15/vote_min=1/price_min=3.0/half_pct=0.10/hard_sl=0.20/trail_sl=0.25')
    log(f'[T4] 入场: 首次 (左试) C3 必选 + [C4/C7/C8] ≥ 1 → 买半仓 (10% 总资) | 加仓 (右确认) 持仓中+未加过+价格>MA10 → 买另半仓 (10% 总资) → 加满 20%')
    log(f'[T4] 出场: 20% 硬止损 (V10 精神) + 25% 峰值回撤止盈 + 破 MA10 快离场 + MACD 死叉 (4 机制)')
    log(f'[T4] 加仓状态机: self.added 字段 (V14 内部设计, 不是 subagent 加的外部 lock)')
    ...
    # V14 策略类 sha256 (用户原版对照)
    import hashlib
    v14_path = '/Users/junze/quant-monitor-local/strategies/goldcombo/goldcombo_strategy_ashare_v14.py'
    with open(v14_path, 'rb') as f:
        v14_sha256 = hashlib.sha256(f.read()).hexdigest()

    summary = {
        'strategy_id': 'goldcombo',
        'strategy_version': 'V14_ScaleIn (GoldComboV14_ScaleIn 用户原版左试右加, 2026-08-16)',
        'strategy_name': '黄金组合A · 沪深 A 股 (排除科创+创业) · V14 左试右加版',
        'data_period': '5Y',
        ...
        'strategy_file_sha256': v14_sha256,
        'entry_logic_v14': '首次 (左试) C3 必选 + [C4/C7/C8] ≥ 1 → 买半仓 (10% 总资) | 加仓 (右确认) 持仓中+未加过+价格>MA10 → 买另半仓 (10% 总资) → 加满 20%',
        'exit_logic_v14': '20% 硬止损 (V10 精神) + 25% 峰值回撤止盈 + 破 MA10 快离场 (新加) + MACD 死叉 (DIFF 下穿 DEA)',
        'config': {
            ...
            'half_pct': 0.10, 'hard_sl': 0.20, 'trail_sl': 0.25,
        ...
        'comparison': {
            ...
            'v14_5y_return_pct': round(total_return_pct, 4),
            'v14_5y_trade_count': int(total_trades),
            'v14_5y_pool': n_pool,
            'v14_5y_worst_dd': round(worst_max_dd, 4),
        },
        'user_predicted': '用户原话 (2026-08-16 派单): 重点观察 总收益 (预期接近或超越 V10 的 +1.6%) / 笔数 (应比 V11 的 12063 少, 因 MA10 加仓过滤) / worst DD (预期 ≤ -15%)',
        'honest_declaration': (
            'V14_ScaleIn 是用户上传的左试右加版 (类名 GoldComboV14_ScaleIn, 首次半仓 + MA10加仓 + 快卖)。'
            'V14 一字不差, 未加任何外部 hold/lock (self.added 是 V14 内部加仓状态机, 不是外部 lock)。'
            'V14 替换 V13_PureRight 旧类, 强制 setcash(50000.0) 锁死 + half_pct=0.10 半仓配置。'
            'V14 vs V13 数据可比因都跑 1950 只沪深 A 股 5Y (排除 688/300 科创+创业)。'
            'V14 与 V13 的核心差异: 入场左试 (C3+≥1 半仓) + MA10站上加满 (右侧确认), 出场 4 机制 (含 20%硬止损+破MA10快离场替换V13的15%硬止损)。'
        ),
    }
