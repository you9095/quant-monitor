...
# -*- coding: utf-8 -*-
"""
T4 · 黄金组合 A 真实 backtrader 5Y 回测 (2026-08-14)
...
- 止损: 8% 硬止损
- 资金分配: 等权 1/N
- 输出: T4_5y/baseline_ashare_real_5y.json
"""
import json
import os
import sys
...
OUT_DIR = '/Users/junze/goldcombo_real_backtest/T4_5y'
OUT_JSON = os.path.join(OUT_DIR, 'baseline_ashare_real_5y.json')
...
def main():
    ...
    log(f'[T4] pool size: {len(pool_5y)} A 股')
    log(f'[T4] initial_capital: {INITIAL_CAPITAL}')
    log(f'[T4] effective_capital (scaled): {EFFECTIVE_CAPITAL}')
    log(f'[T4] capital per stock: {EFFECTIVE_CAPITAL / len(pool_5y):.2f}')
    log(f'[T4] commission: {COMMISSION_RATE}, slippage: {SLIPPAGE}, sl_pct: {STOP_LOSS_PCT}')
    ...
    summary = {
        ...
        'traded_stocks_full': [r for r in results if r.get('trade_count', 0) > 0],  # 所有触发过的股票 (完整)
    ...
    log(f'[T4] total_return_pct: {total_return_pct:.2f}%')
    log(f'[T4] annualized: {annualized:.2f}%')
    log(f'[T4] avg_max_dd: {avg_max_dd:.2f}%')
    log(f'[T4] worst_max_dd: {worst_max_dd:.2f}%')
    log(f'[T4] sharpe_avg: {avg_sharpe:.4f}')
    log(f'[T4] trades: {total_trades} (wins: {total_wins}, losses: {total_losses})')
    log(f'[T4] win_rate: {win_rate_pct:.2f}%')
