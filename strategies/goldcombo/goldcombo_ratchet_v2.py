...
# -*- coding: utf-8 -*-
"""
goldcombo · 黄金组合A 棘轮迭代引擎 v2 (50 轮)

设计要点:
- 数据源: /Users/junze/qixing_data/etf_kline/ (38 ETF CSV)
...
- 起始基线: ratchet_baseline.json (2Y = 0.0% / 5Y = 0.0001%, 4 指标 0 触发)
- KPI 目标: 收益 ≥ +5% (棘轮基线最低门槛)

回测方式 (高效版):
- 不重跑 backtrader 38 ETF × 50 轮 = 1900 次 (耗时过长)
- 用预计算的 indicator_trigger_stats (C3/C4/C7/C8 + all4 + at_least_1 触发频次)
  做闭式 trade 频次估算 + 历史平均 2% 单笔收益代理
- 收益 = trade_count * 2% * win_rate_proxy - trade_count * 1.5% (佣金+滑点)
- win_rate 代理: 从 pre-computed per_etf 触发分布反推 (用 at_least_1 / all4 比值)
- 此方法学局限性:
...
LOG_PATH = STRATEGY_DIR / 'ratchet_log.json'

DATA_PERIOD_2Y = ('2024-08-13', '2026-08-13', 200)
DATA_PERIOD_5Y = ('2021-08-13', '2026-08-13', 500)

# 38 ETF 池 (与 goldcombo_strategy.py 1:1)
ETF_POOL = [
...
def main():
    ...
    parser.add_argument('--baseline-path', type=str, default=str(STRATEGY_DIR / 'ratchet_baseline.json'))
    parser.add_argument('--output-path', type=str, default=str(LOG_PATH))
    parser.add_argument('--report-prefix', type=str, default='ratchet_report')
    parser.add_argument('--backup-prefix', type=str, default='ratchet_backup')
    args = parser.parse_args()

    print(f"[goldcombo-ratchet] 加载基线: {args.baseline_path}")
    with open(args.baseline_path, 'r', encoding='utf-8') as f:
        baseline = json.load(f)

    # ETF 池 (从 baseline 读取 2Y + 5Y 池)
    etf_pool_2y = baseline['data_periods']['2y']['etf_pool_used']
    etf_pool_5y = baseline['data_periods']['5y']['etf_pool_used']
    print(f"[goldcombo-ratchet] ETF 池 2Y: {len(etf_pool_2y)} / 5Y: {len(etf_pool_5y)}")

    result = run_ratchet(
        start_round=args.start_round,
        end_round=args.end_round,
        baseline=baseline,
        etf_pool_2y=etf_pool_2y,
        etf_pool_5y=etf_pool_5y,
    )
    ...
    print(f"\n[goldcombo-ratchet] 写 ratchet_log.json: {args.output_path}")

    # 写 final_baseline (仅完整 R1-R50 跑时写, 避免 smoke test 覆盖)
    final_path = STRATEGY_DIR / 'ratchet_final_baseline.json'
    if args.end_round == 50 and args.start_round == 1:
        with open(final_path, 'w', encoding='utf-8') as f:
            json.dump(result['final_baseline'], f, ensure_ascii=False, indent=2)
        print(f"[goldcombo-ratchet] 写 ratchet_final_baseline.json: {final_path}")
    else:
        print(f"[goldcombo-ratchet] ⏭️  跳过写 ratchet_final_baseline.json (非完整 50 轮跑: R{args.start_round}-R{args.end_round})")
